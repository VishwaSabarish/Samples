    ...
    public abstract class BaseObject<T>
    {
        ...
        public bool Save(string UpdateWhere = "")
        {
            ...
            foreach (var property in typeof(T).GetProperties())
            {
                ...
                Parameters.Add(new MySqlParameter("@" + property.Name.ToCustomString(), property.GetValue(property)));
            }
            ...
            if (ID > 0)
            {
                foreach (var property in typeof(T).GetProperties())
                {
                    if (property.Name.ToLower() == identitycol.ToLower())
                    {
                        var item = Activator.CreateInstance<T>();
                        Type convertTo = Nullable.GetUnderlyingType(property.PropertyType) ?? property.PropertyType;
                        property.SetValue(item, Convert.ChangeType(ID, convertTo), null);
                    }
                }
            }
